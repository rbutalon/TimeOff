<?php
  ...
  $sql = "SELECT leave_type, start_date, end_date, status, created_at, approved_by 
          FROM leave_tbl 
          WHERE status IN ('Approved', 'Rejected') AND user_id = '$userID'
...
<html lang="en">
  <head>
    ...
    <link rel="stylesheet" href="./styles/components/employee-dashboard/notifications.css">
  ...
  <body>
    ...
    <div class="body__wrapper">
      <nav class="sidebar" id="sidebar">
        ...
        <ul class="sidebar__middle">
          ...
          <li>
            <a href="" class="sidebar__item notifications active">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="size-6">
      ...
      <main class="dashboard">
          ...
          <section class="request__wrapper formContainer section" id="request__wrapper">
                <?php if (count($notifications) > 0): ?>
                  <?php foreach ($notifications as $notif): ?>
                    <?php
                    $status = strtolower($notif['status']);
                    $statusText = ucfirst($status);
                    $statusColor = ($status === 'approved') ? '#2c8e71' : '#b34141';
                    ?>
                    <div class="alert-card">
                      <div class="alert-date"><?php echo date("F j, Y", strtotime($notif['created_at'])); ?></div>
                      <div class="alert-body">
                        <div class="alert-icon" style="background-color: <?php echo $statusColor; ?>;">!</div>
                        <div class="alert-text">
                          <p>
                            <strong>
                              Your <?php echo htmlspecialchars($notif['leave_type']); ?> leave request from
                              <?php echo date("F j", strtotime($notif['start_date'])); ?> to
                              <?php echo date("F j, Y", strtotime($notif['end_date'])); ?> has been
                              <?php echo $statusText; ?>.
                            </strong>
                          </p>
                          <p class="alert-approver"><?php echo htmlspecialchars($notif['status']); ?> by:
                            <?php echo htmlspecialchars($notif['approved_by']); ?>
                          </p>
                        </div>
                      </div>
                    </div>
                  <?php endforeach; ?>
                <?php else: ?>
                  <p class="no-alerts">No approved or rejected leave requests found.</p>
                <?php endif; ?>
          </section>
      </main>
    </div>
    ...
    <script src="./src/employee/leaveRequests.js"></script>
