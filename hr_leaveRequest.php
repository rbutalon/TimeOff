...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && strpos($_SERVER["CONTENT_TYPE"], "application/json") !== false) {
  ...
  // get approver's name from session
  if (!isset($_SESSION['fName'])) {
    http_response_code(403);
    echo json_encode(["error" => "Approver not authenticated."]);
    exit;
  }

  $approved_by = $_SESSION['fName'];

  // update leave status and approved_by in the database
  $stmt = $conn->prepare("UPDATE leave_tbl SET status = ?, approved_by = ? WHERE leave_id = ?");
  if (!$stmt) {
  ...
  $stmt->bind_param("ssi", $status, $approved_by, $leave_id);

  if ($stmt->execute()) {
    echo json_encode(["message" => "Leave request #$leave_id has been $status by $approved_by."]);
  } else {
    http_response_code(500);
...
$sql = "SELECT leave_tbl.*, users_tbl.fName, users_tbl.role, users_tbl.department
        FROM leave_tbl
        JOIN users_tbl ON leave_tbl.user_id = users_tbl.userID
        WHERE leave_tbl.status = 'pending'
...
<html lang="en">
  ...
  <body>
              ...
              <?php foreach ($leaves as $leave): ?>
                ...
                <div class="card__details">
                  ...
                  <div class="card__actions">
                    <button class="btn-approve approveBtn" data-id="<?php echo $leave['leave_id'] ?>">Approve</button>
                  <button class="btn-reject rejectBtn" data-id="<?php echo $leave['leave_id'] ?>">Reject</button>
                  </div>
                </div>
              </div>
              <?php endforeach; ?>
    ...
    <script src="./src/approver/requestValidation.js"></script>
